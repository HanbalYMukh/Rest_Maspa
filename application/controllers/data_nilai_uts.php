<?php
	class data_nilai_uts extends MY_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('nilai_uts_model');
	...
	function  index ($kelas)
	{
		...
		$data['fields']	= $this->nilai_uts_model->get_list_data($kelas);
		$data['kelas'] = $kelas;
		...
		$this->template->build('data/nilai_uts_view', $data);
	...
	function addData($kelas)
	{
		if($_POST==NULL)
		{
			...
			$data['thn_ajaran']= $this->tahunajaran_model->selectAll();
			$data['kelas']= $kelas;
			$this->template->build('inc/nilai_uts_add', $data);
		}else {
			...
			/*<SELECT DISTINCT> Perlu dibuat rekondisi, Jika nilai dengan nis,  Kelas, Pelajaran, Tahun Ajaran dan Smester sudah ada di database, maka ada action lain untuk menanggulangi terjadinya perubahan data tanpa disengaja*/
				...
				$this->nilai_uts_model->insert($nis,$_POST['mapel'],$kelas,$_POST['thn_ajaran'],$_POST['semester'],$isi);
				$data['hasil']= $this->nilai_uts_model->dataNilai($kelas,$_POST['mapel'],$_POST['thn_ajaran'],$_POST['semester']);
				$this->template->build('data/nilai_uts_input_view',$data);
				}
			}
		}
	}
	function detailData($nis){
			...
			$data['field'] = $this->nilai_uts_model->selectSiswa($nis);
			$data['nis']=$nis;
			$this->template->build('data/detil_uts_siswa_view', $data);
	}
	
	function viewSpesifik(){
		$data['detail'] = $this->nilai_uts_model->dataNilai($_GET['kelas'],$_GET['mapel'],$_GET['thn_ajaran'],$_GET['semester']);
		$this->load->view('data/spesifik_uts_view',$data);
	}
	function viewDetail(){
		$data['nilai'] = $this->nilai_uts_model->ambilNilai($_GET['nis'],$_GET['thn_ajaran'],$_GET['semester']);
		$this->load->view('data/detil_spesifik_uts_siswa_view',$data);
	}
	function edit(){
			$this->nilai_uts_model->update();
			$data=$this->nilai_uts_model->selectId($_POST['id']);
			echo json_encode ($data);
		}
	function hapus($id){
		$this->nilai_uts_model->delete($id);
	}
}
