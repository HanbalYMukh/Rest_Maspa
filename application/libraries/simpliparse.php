...
Class Simpliparse
{
	function Simpliparse()
	{
		...
		$this->SP->load->library(array('loader'));
		$this->SP->load->library(array('pagination'));
	}
	...
	function getAjaxPagination($total_page,$per_page,$url,$uri=3,$update)
	{
		...
		$data = $this->SP->ajaxpagination->create_links();
		return $data;
	}
	...
	function build_select_day($name)
	{
		...
		echo '<select name=\''.$name.'\'><option value=\'00\'>::Tgl::</option>'
			.$result.
			'</select>';
	}
	function build_select_month($name)
	{	
		$result='';
		...
		echo '<select name=\''.$name.'\'><option value=\'00\'>:: Bulan ::</option>'
			.$result.
			'</select>';
	} 
	function build_select_year($name)
	{
		...
		echo '<select name=\''.$name.'\'><option value=\'00\'>:: Tahun ::</option>'
			.$result.
			'</select>';
	}
	function build_select_year_up($name)
	{
		$result='';
	...
	function build_select_common($name,$data,$value,$mid,$title)
	{
		$result='';
		if(is_array($mid))
		{
			...
			{	
			$opt='<option value=\''.$row->$value.'\' >'.$row->$pm.' - '.
			$row->$pm2.'</option>';
			$result=$result.$opt;
			}
		}else{
			...
			.$this->SP->validation->set_select($name,$row->$value).' >'.$row->$mid.'</option>';
				$result=$result.$opt;
			}
		}
		echo '<select name=\''.$name.'\'><option value=\'\'>'.$title.'</option>'
			.$result.
			'</select>';
	}	
	function build_select_common_array($name,$data,$value,$mid,$title,$id)
	{
		...
		echo '<select id=\''.$id.'\' sname=\''.$name.'\'><option value=\'\'>'.$title.'</option>'
			.$result.
			'</select>';
	}	
	...
	function build_select_month_edit($name,$key)
	{	
		$result='';
		$namabulan=array(1=>"Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus",
				"September","Oktober","November","Desember");
	...
	function build_select_year_edit($name,$key)
	{
		$result='';
		for($k=1930;$k<=date("Y");$k++)
		{			   
	...
	function build_select_common_edit($name,$data,$value,$mid,$title,$key)
	{
		$result='';
		if(is_array($mid))
		{
